...
After esbuild finishes a build, the plugin examines its in-memory output files
and adds Gzip and Brotli variants for supported asset types. The build tool then
emits those variants alongside the original assets:
...
For an Angular application built with `@nx/angular:application`, register the
...
The application builder writes these generated `.gz` and `.br` files into its
final browser output directory (by default, `dist/apps/ui/browser`). The sample
skips the injected `env-config` bundle, while compressing JavaScript, CSS, and
HTML.
